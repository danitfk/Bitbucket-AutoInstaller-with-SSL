...
echo "4) Install MySQL Driver into Bitbucket..." &&  mysql_dirver_install > /dev/null && echo "$(tput setaf 2)4) MySQL Driver Installed successfully. $(tput sgr 0)"
echo "5) Configure MySQL Database..." &&  mysql_configure > /dev/null && echo "$(tput setaf 2)5) MySQL Database configured successfully. $(tput sgr 0)"
echo "6) Install Let's Encrypt and Issue SSL..." &&  install_letsencrypt > /dev/null && echo "$(tput setaf 2)6) Let's Encrypt install and SSL certificate issued successfully. $(tput sgr 0)"
