...
echo "2) Installing Oracle Java JDK 8 ..." &&  java_install $(tput setaf 3) > /dev/null && echo "$(tput setaf 2)2) Oracle Java JDK 8 installed successfully. $(tput sgr 0)"
echo "3) Create bitbucket user and set permissions..." &&   user_permissions $(tput setaf 3) > /dev/null && echo "$(tput setaf 2)3) Bitbucket user created successfully. $(tput sgr 0)"
echo "4) Install MySQL Driver into Bitbucket..." && mysql_dirver_install $(tput setaf 3) > /dev/null && echo "$(tput setaf 2)4) MySQL Driver Installed successfully. $(tput sgr 0)"
echo "5) Configure MySQL Database..." &&  mysql_configure $(tput setaf 3) > /dev/null && echo "$(tput setaf 2)5) MySQL Database configured successfully. $(tput sgr 0)"
echo "6) Install Let's Encrypt and Issue SSL..." &&  install_letsencrypt $(tput setaf 3) > /dev/null && echo "$(tput setaf 2)6) Let's Encrypt install and SSL certificate issued successfully. $(tput sgr 0)"
